...
function site-manager@center-text {
    ...
    for ((i = 0; i < PADDING_WIDTH / (${#PATTERN} * 2); i++)); do
        PADDING+="${PATTERN}"
    done
...
function site-manager@show-items-list {
    ...
    for ITEM in "${ITEMS_ARRAY_REF[@]}"; do
...
function site-manager@action-selector {
    ...
    if [ "$USER_INPUT" -ge 1 ] && [ "$USER_INPUT" -le "${#ACTIONS_ARRAY_REF[@]}" ]; then
...
function site-manager:_file-selector {
    ...
    local ACTION_NAME="$2"
    if [ $ACTION_NAME != "ENABLE" ] && [ $ACTION_NAME != "DISABLE" ]; then
    ...
    if [ "$USER_INPUT" -ge 1 ] && [ "$USER_INPUT" -le "${#FILES_ARRAY[@]}" ]; then
        if [ "${FILES_ARRAY[$USER_INPUT - 1]}" = "site-manager:selector:main" ]; then # IF user select LAST point then execute it
            "${FILES_ARRAY[$USER_INPUT - 1]}"
        else
            if [ $ACTION_NAME = "ENABLE" ]; then # IF this enabling command
                site-manager:site:_enable "${FILES_ARRAY[$USER_INPUT - 1]}"

                site-manager@main
            elif [ $ACTION_NAME = "DISABLE" ]; then # IF this disabling command
...
function site-manager:site:_enable-all-from {
    ...
    for ONE_ENABLED_FILE in "${ITEMS_ARRAY_REF[@]}"; do
